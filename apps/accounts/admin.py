...
@admin.action(description="Reset 2FA devices (SUPERUSERS ONLY)")
def reset_2fa_devices(modeladmin, request, queryset):
    """
    Deletes OTP devices for selected users.
    This is a recovery tool. Restrict to superusers to prevent abuse.
    """
...
class UserAdmin(BaseUserAdmin):
    ...
    actions = [reset_2fa_devices]

    fieldsets = (
        (None, {"fields": ("email", "password")}),
        (_("Personal info"), {"fields": ("first_name", "last_name", "phone_number")}),
        (_("User Type"), {"fields": ("user_type", "employee_role", "employee_id")}),
        (_("Customer Info"), {"fields": ("customer_number", "company_name"), "classes": ("collapse",)}),
        (_("Permissions"), {
            "fields": (
                "is_active", "email_verified", "is_approved",
...
class CustomerPricingTierAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("user", "tier", "is_active")}),
        (_("Dates"), {"fields": ("effective_date", "expiration_date")}),
        (_("Metadata"), {"fields": ("created_by", "created_at"), "classes": ("collapse",)}),
    )
