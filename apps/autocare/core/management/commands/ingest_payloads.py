...
from typing import Any, Dict, List, Optional, Type, Tuple, Iterable, Set
...
from apps.autocare.ingest.plans import get_dataset, EndpointSpec
...
class PlanItem:
    """
    Adapt your new EndpointSpec to the shape the ingest runner expects.
    """
    spec: EndpointSpec
    model: str  # Django model name (string)

    @property
    def endpoint(self) -> str:
        # Keep a single “display endpoint”
        return self.spec.request_path

    @property
    def endpoint_keys(self) -> Set[str]:
        """
        All endpoint strings we will accept for matching AutocareRawRecord.endpoint.
        AutocareRawRecord.endpoint in your system may be stored as:
        - "Year"
        - "/vcdb/Year"
        - "/api/v1/vcdb/Year"
        - etc.
        """
        db = self.spec.db
        res = self.spec.resource
        v = self.spec.api_version
        return {
            res,  # "MetaData"
            f"{db}:{res}",  # "padb:MetaData"  <-- YOUR STORAGE FORMAT
            f"/{db}/{res}",  # "/padb/MetaData"
            f"/{v}/{db}/{res}",  # "/v4/padb/MetaData"
            self.spec.request_path,  # "/api/v4/padb/MetaData"
        }
...
def normalize_value(v: Any) -> Any:
    # IMPORTANT: empty strings from API payloads must become NULL.
    if v in ("null", "", "None"):
        return None
...
def build_column_map(model: Type[models.Model]) -> Dict[str, str]:
    """
    Normalized API key (no underscores, lowercase) -> django attribute
    """
    ...
    for field in model._meta.fields:
        ...
        if field.is_relation and field.many_to_one:
            if field.db_column:
                mapping[field.db_column.replace("_", "").lower()] = field.attname
            continue

        # Real DB columns
        if field.db_column:
            mapping[field.db_column.replace("_", "").lower()] = field.name
        else:
            # API metadata fields (culture_id etc)
            mapping[field.name.replace("_", "").lower()] = field.name
...
def _vehicle_fk_fields(model: Type[models.Model]) -> list[models.ForeignKey]:
    """
    Return FK fields on model that point to Vehicle (robust label match).
    """
    try:
    ...
    vehicle_label = VehicleModel._meta.label_lower  # e.g. "autocare.Vehicle" -> "autocare.vehicle"

    fks: list[models.ForeignKey] = []
    for f in model._meta.fields:
        ...
        if rel_model is None:
            continue
        if rel_model._meta.label_lower == vehicle_label:
            fks.append(f)
...
def filter_rows_missing_engine_config2(
    ...
    """
    Skip rows whose EngineConfig2 FK references are missing.
    We only apply this for models that actually have an FK field with attname engine_config2_id.
    """
    ...
    # FIX: do not assume engine_config_id field name — use PK name.
    pk = EngineConfig2Model._meta.pk.name
    existing = set(
        EngineConfig2Model.objects.filter(**{f"{pk}__in": referenced}).values_list(pk, flat=True)
    )
...
class Command(BaseCommand):
    help = (
        "STRICT Autocare ingest (offline replay from AutocareRawRecord).\n"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="auto_fk",
            help="explicit = dataset plan order; auto_fk = topo sort by FK graph",
        )
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="Skip rows whose EngineConfig2 FK references are missing, and log them.",
        )
    ...
    def handle(self, *args, **opts):
        ...
        # Clear previous skip + summary logs on each run
        ...
        # =====================================================
        # BUILD PLAN FROM NEW plans.py REGISTRY
        # =====================================================
        dataset = get_dataset(opts["db"])

        plan_items: list[PlanItem] = []
        for spec in dataset.plan:
            # support --endpoint as either "Year" or "/api/v1/vcdb/Year" or "/vcdb/Year"
            if opts["endpoint"]:
                wanted = opts["endpoint"].strip()
                acceptable = {
                    spec.resource,
                    spec.request_path,
                    f"/{spec.db}/{spec.resource}",
                }
                if wanted not in acceptable:
                    continue

            if not spec.django_model:
                self.stderr.write(f"⚠ No django_model for {spec.key} ({spec.request_path}) — skipping")
                continue

            if resolve_model(app_label, spec.django_model) is None:
                self.stderr.write(f"⚠ Missing model for {spec.key}: {spec.django_model}")
                continue

            plan_items.append(PlanItem(spec=spec, model=spec.django_model))

        if not plan_items:
            self.stdout.write("No ingestable endpoints found.")
            return

        # Order: explicit = dataset.plan order; auto_fk = topo sort.
        if opts["order"] == "explicit":
            ordered = plan_items
        else:
            model_names = [p.model for p in plan_items]
            sorted_models = topo_sort_models(app_label, model_names)
            index = {m: i for i, m in enumerate(sorted_models)}
            ordered = sorted(plan_items, key=lambda p: index.get(p.model, 10**9))
        ...
        for log in qs:
            ep = (log.endpoint_key or "").strip()
            logs_by_endpoint[ep].append(log)

            # handle "padb:MetaData"
            if ":" in ep:
                logs_by_endpoint[ep.split(":", 1)[1]].append(log)

            # handle "/api/v4/padb/MetaData"
            if "/" in ep:
                logs_by_endpoint[ep.rsplit("/", 1)[-1]].append(log)
        ...
        for item in ordered:
            ...
            logs: list[AutocareRawRecord] = []
            for k in item.endpoint_keys:
                logs.extend(logs_by_endpoint.get(k, []))
            ...
            if not logs:
                self.stdout.write(f"{item.spec.resource}: no raw data")
                continue

            self.stdout.write(f"\n▶ {item.spec.request_path} → {item.model}")
        ...
        # End summary
        summary_logger.info("===== INGEST SUMMARY =====")
    ...
    def _truncate_models(self, app_label: str, ordered: list[PlanItem]) -> None:
        """
    ...
    def _ingest_log_strict(
        ...
        for idx, row in enumerate(payload):
            ...
            lower = {_norm_key(str(k)): v for k, v in row.items()}
        ...
        # FILTER/SKIP: MISSING ENGINECONFIG2
        ...
        # INSERT (bulk, then isolate on IntegrityError)
                ...
                for obj in chunk:
                    try:
                        ...
                        raise RuntimeError(
                            ...
                            f"(endpoint={log.endpoint_key}, log_id={log.id}). "
                            f"See {ERROR_LOG_PATH}"
                        ) from row_exc
